class Person
  module Friend
    ...
    def can_request_friendship_with?(person)
      Setting.get(:features, :friends) and person != self  and !friend?(person)  and person.valid_email? and person.friends_enabled and !friendship_rejected_by?(person) and !friendship_waiting_on?(person)
    end
