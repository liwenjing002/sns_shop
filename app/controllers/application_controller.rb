class ApplicationController < ActionController::Base
  ...
  #获得访问历史，包括 用户，群组和place    
  def get_visit_history
#    session[:visit]= nil
    if !session[:visit]
      session[:visit] = Array.new 
      session[:visit].push(@logged_in)  #默认第一个返回我的主页
    end
    model_name =  params[:controller].gsub(/\b\w/) { $&.upcase }.chomp("s")
    model_name = "Person" if params[:controller] =="people"
    visit = eval(model_name+".find_by_id(#{params[:id]})")
   unless session[:visit].include? visit
    if session[:visit].length < 4
      session[:visit].push(visit) if visit
    else
      session[:visit].drop(0)
      session[:visit].push(visit)  if visit
    end
    end
    @visit = session[:visit]
